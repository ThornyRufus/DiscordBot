...
bot = commands.Bot(command_prefix=PREFIX, intents=discord.Intents(voice_states=True, guilds=True, guild_messages=True, message_content=True, reactions=True))
...
@bot.command(name='example', aliases=['e'])
async def ex(ctx: commands.Context, *args):
    await ctx.send("example")
    
@bot.command(name='ip')
async def ip(ctx: commands.Context, *args):
    os.system('sh /home/maurice/ipscript.sh')
    with open('/home/maurice/ip.txt','r') as file:
        ip = file.read().replace('\n','')
    await ctx.send(str(ip))
    await bot.change_presence(activity=discord.Game(name=str(ip)))
    
@bot.command(name='start_server')
async def start_server(ctx: commands.Context, *args):
    os.system('bash /home/maurice/start.sh SERV')
    await message.channel.send("starting server")
    
    
@bot.command(name='stop_server')
async def stop_server(ctx: commands.Context, *args):
    os.system('bash /home/maurice/stop.sh')
    await message.channel.send("stopping server")

@bot.command(name='conf')
async def conf(ctx: commands.Context, *args):
    with open('/home/maurice/conf.txt','r') as file:
        conf = file.read().replace('\n','')
    await ctx.send("conf : " + conf)
...
async def play(ctx: commands.Context, *args):
    ...
    with yt_dlp.YoutubeDL({'format': 'worstaudio',
        ...
        try: queues[server_id].append((path, info))
            ...
            connection.play(discord.FFmpegOpusAudio(path), after=lambda error=None, connection=connection, server_id=server_id: after_track(error, connection, server_id))


@bot.command(name='blindtest', aliases=['bt'])
async def blindtest(ctx: commands.Context, *args):
    vemoji = '\U00002705'
    xemoji = '\U0000274C'
    ...
    file = random.choice(os.listdir("/home/maurice/AudioFiles/bt"))
    #while (file in queue):
    #    file = random.choice(os.listdir("/home/maurice/AudioFiles/bt"))
    #queue.append(file)
    
    await ctx.send("Devine la zik !")
    voice_state = ctx.author.voice
    ...
    source = discord.FFmpegOpusAudio("/home/maurice/AudioFiles/bt/"+file)
    
    connection.play(source, after=lambda error=None, connection=connection, server_id=server_id: after_track(error, connection, server_id))
    
    voice_client = get_voice_client_from_channel_id(ctx.author.voice.channel.id)
    await asyncio.sleep(15)
    voice_client.pause()
    message = await ctx.send("Alors, trouvé ?")
    await message.add_reaction(vemoji)
    await message.add_reaction(xemoji)
        
    try:
        reaction, user = await bot.wait_for('reaction_add', check=None, timeout = 30.0)
        
        if reaction.emoji == vemoji:
            await ctx.send("gg, réponse : "+file)
        
        elif reaction.emoji == xemoji:
            await ctx.send("20s de plus alors")
            voice_client.resume()
            await asyncio.sleep(20)
            await ctx.send("réponse : "+file)
            voice_client.stop()
        
        else:
            await ctx.send("error : "+file)
        
    except asyncio.TimeoutError:
        # at this point, the check didn't become True.
        await ctx.send(f"**{ctx.author}**, timeout.")
        return
    
@bot.command(name='blindtest_add', aliases=['bta'])
async def bta(ctx: commands.Context, *args):
    query = ' '.join(args)
    # this is how it's determined if the url is valid (i.e. whether to search or not) under the hood of yt-dlp
    will_need_search = not urllib.parse.urlparse(query).scheme
    ...
    with yt_dlp.YoutubeDL({'format': 'worstaudio',
                           ...
                           # 'progress_hooks': [lambda info, ctx=ctx: video_progress_hook(ctx, info)],
                           # 'match_filter': lambda info, incomplete, will_need_search=will_need_search, ctx=ctx: start_hook(ctx, info, incomplete, will_need_search),
                           'paths': {'home': f'/home/maurice/AudioFiles/bt/'}}) as ydl:
        try:
            info = ydl.extract_info(query, download=False)
